...
@login_required
def index(request):
    return render_to_response("adminboard/index.html", {"user_name": request.session["user_name"]})
...
def user_list(request):
    ...
    return render_to_response("adminboard/user_list.html",
                {"user_list": user_list})
...
def ajax_load_site_checking_details(request):
    ...
    c_site_checking_daemon_logs = mongo_client.getSiteDBCollection(site_id, "site_checking_daemon_logs")
...
def site_checking_details(request):
    ...
    return render_to_response("adminboard/site_checking_details.html",
...
class AddSiteHandler(BaseSiteHandler):
    ...
    def __call__(self, request):
        if request.method == "GET":
            return render_to_response("adminboard/edit_site.html", 
...
class EditSiteHandler(BaseSiteHandler):
    ...
    def _handleGET(self, request):
        ...
        site["disabledFlowsFormatted"] = self._formatDisabledFlows(site.get("disabledFlows", ""))
        return render_to_response("adminboard/edit_site.html", 
...
def login(request):
    if request.method == "GET":
        ...
        return render_to_response("adminboard/login.html", {"msg": msg}, 
                  context_instance=RequestContext(request))
    else:
        users = mongo_client.getTjbDb()["admin-users"]
        ...
        if login_succ:
            ...
            return redirect(reverse("login") + "?msg=login_failed")


import copy
def _getCurrentUser(request):
    if request.session.has_key("user_name"):
        return mongo_client.getTjbDb()["admin-users"].find_one({"user_name": request.session["user_name"]})
    else:
        return None
