...
@log_function
def calc_hot_keywords(site_id, connection, client):
    client.execute("DROP TABLE keyword_counts")
    client.execute("CREATE TABLE keyword_counts ( "
                   " date_str STRING, "
                   " keyword STRING, "
                   " count    INT "
                   ")")

    client.execute("INSERT OVERWRITE TABLE keyword_counts "
                   "SELECT date_str, keyword, COUNT(*) "
